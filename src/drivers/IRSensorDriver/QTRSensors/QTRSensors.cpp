...
void QTRSensors::emittersSelect(QTREmitters emitters) {
  ...
  switch(emitters) {
  case QTREmitters::Odd: offEmitters = QTREmitters::Even; break;

  case QTREmitters::Even: offEmitters = QTREmitters::Odd; break;

  case QTREmitters::All: emittersOn(); return;

  case QTREmitters::None: emittersOff(); return;
...
void QTRSensors::setCalibrationOn(uint16_t maxChannel[],
                                  uint16_t minChannel[]) {
  // (Re)allocate and initialize the arrays if necessary.
  if(!calibrationOn.initialized) {
    uint16_t *oldMaximum  = calibrationOn.maximum;
    calibrationOn.maximum = (uint16_t *)realloc(
        calibrationOn.maximum, sizeof(uint16_t) * _sensorCount);
    ...
    calibrationOn.minimum = (uint16_t *)realloc(
        calibrationOn.minimum, sizeof(uint16_t) * _sensorCount);
...
void QTRSensors::calibrateOnOrOff(CalibrationData &calibration,
  ...
  if(!calibration.initialized) {
    ...
    calibration.maximum  = (uint16_t *)heap_caps_realloc(
        calibration.maximum, sizeof(uint16_t) * _sensorCount, MALLOC_CAP_8BIT);
    ...
    calibration.minimum  = (uint16_t *)heap_caps_realloc(
        calibration.minimum, sizeof(uint16_t) * _sensorCount, MALLOC_CAP_8BIT);
...
void QTRSensors::readCalibrated(uint16_t *sensorValues, QTRReadMode mode) {
  if(!calibrationOn.initialized) return;
...
void QTRSensors::setMultiplexerDigitalAddress(std::bitset<4> address) {
  ...
  for(int i = 0; i < address.size(); i++) {
    if(address[i]) {
      gpio_set_level((gpio_num_t)_sensorPinsMultiplexer[i], 1);
    } // se bit=1, nível lógico alto
    else {
      gpio_set_level((gpio_num_t)_sensorPinsMultiplexer[i], 0);
  ...
  case QTRType::Analog:
    ...
    for(uint8_t j = 0; j < _samplesPerSensor; j++) {
      for(uint8_t i = start; i < _sensorCount; i += step) {
        ...
        esp_err_t err;
        do {
          err = adc_oneshot_read(adc1_handle, (adc_channel_t)_sensorPins[i],
                                 &adc_raw);
...
QTRSensors::~QTRSensors() {
  ...
  if(_sensorPinsMultiplexer) {
    free(_sensorPinsMultiplexer);
  }
  if(calibrationOn.maximum) {
